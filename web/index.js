...
function updateText(element, text) {
	return new Promise((resolve) => {
		...
		setTimeout(function () {
			...
			resolve({ value: true }); // Thêm return value
		}, 300);
	});
}
	...
	if (imageData !== undefined) {
		if (imageData.width === newImageData.width && imageData.height === newImageData.height) {
			if (typeof pixelmatch === "function") {
				numDiffPixels = pixelmatch(imageData.data, newImageData.data, null, imageData.width, imageData.height, {
					threshold: 0.1,
				});
				if (numDiffPixels < 10) {
					sameImage = true;
...
function recognize_image(image) {
	...
	(async () => {
		...
		if (response.result) {
			...
			// Thêm return value để đảm bảo Eel nhận được giá trị
			const result = await updateText(output, response.result);
		...
		return { value: false }; // Thêm return value khi không có result
	})();
}
/*
 *
