...
async function loadGameScripts() {
	...
	gameScripts.forEach((gameScript) => {
...
function createCardSectionElement(iconName, field, value, contentEditable = false, footerIcon = "") {
    ...
    <span class="card_${field}_container mdl-list__item-primary-content">
        ...
        <span ${
					contentEditable && `oninput="changeLogText(this)"`
				} contentEditable=${contentEditable} class="card_${field}">${value}</span>
...
function formatMatchScriptMenu(logId, matchScriptContent) {
    ...
    ${
      ...
      ${log.matches.map((match) => {
				return `<li class="mdl-list__item" log_id="${logId}" onclick="replaceLogText(this.getAttribute('log_id'), this.innerText)">
...
function resetMainWindowState() {
	eel.reset_main_window_state()();
	notify("Đã reset trạng thái cửa sổ chính");
}
